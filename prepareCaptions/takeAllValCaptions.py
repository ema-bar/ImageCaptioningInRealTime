...
#THERE ARE SOME "\n" IN COCO'S VALCAPTIONS2014 FILE ( THERE ARE VERY FEW CASES ) SO CHECK BEFORE STARTING THE "cleanBlankLines.py" FILE
path = ".../coco/captions/"
ids = []
captions = []
file_name = []
fileCaptionsVal = open(path + "tempValCaptions.txt", "w")
    ...
    if item['file_name'] in l:
      ...
      for item2 in data['annotations']:
        if id == item2['image_id']:
          ...
          fileCaptionsVal.write(item['file_name'] + "#" + str(count) + "\t" + caption + "\n")
          count += 1
      count = 0
